...
import {
  resetApplicationRes, resetCreateApplicationErrorMessage,
} from '../../features/dataSlice';
import './newApplication.css';
import { useNavigate } from 'react-router-dom';

const AddressForm = lazy(() => import(
  '../../components/CreateApplicationComponents/AddressForm/AddressForm'
  ));
const ApplicationStatus = lazy(() => import(
...
const NewApplication = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'district2' && !state.city?.name?.includes('Бишкек') && value?.VALUE?.includes('Бишкек')) return;
  ...
  const getDist2 = (dist2name) => {
    ...
    if (dist2name === 'с. Манас') return dists.filter((bxSquare) => bxSquare.VALUE === 'Село Манас')[0];
  ...
  const onSubmit = async (e) => {
    ...
    if ('discount' in finalData && finalData.discount === null) delete finalData.discount;
