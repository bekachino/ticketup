...
const AddressForm = ({
  ...
  return (
    <>
      ...
      >
        <Tab value="house" label="Частный сектор" />
        <Tab value="flat" label="Этажка" />
      </Tabs>
      <Autocomplete
              ...
              onChange={(_, value) => {
                handleChange({
                  target: {
                    name: 'street',
                    value:
                      streets?.find((street) => street?.name === value) || null,
                  },
                });
              }}
      ...
      {addressType === 'house' && state?.district && (
        <TextField
          label="Дом"
          variant="outlined"
          name="exactAddress"
          ...
          onChange={(_, value) => {
            handleChange({
              target: {
                ...
                value:
                  bxSquares[
                    Object.keys(bxSquares).filter((key) =>
                      state?.region?.name
                        ?.toLowerCase()
                        ?.includes(key.toLowerCase())
                    )[0]
                  ]?.find((district2) => district2?.VALUE === value) || null,
              },
            });
          }}
          options={
            bxSquares[
              Object.keys(bxSquares).filter((key) =>
                state?.region?.name?.toLowerCase()?.includes(key.toLowerCase())
              )[0]
            ]?.map((district2) => district2?.VALUE) || []
          }
          loading={bxSquaresLoading}
          loadingText="Загрузка..."
          renderInput={(params) => (
            <TextField {...params} label="Наименование локации" />
          )}
        />
      )}
    </>
