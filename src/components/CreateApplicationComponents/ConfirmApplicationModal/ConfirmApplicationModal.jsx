...
const ConfirmApplicationModal = ({
  ...
  const { createApplicationLoading } = useAppSelector(
    (state) => state.dataState
  );
  const passport1Url = useMemo(
    () => (state?.passport1 ? URL.createObjectURL(state.passport1) : ''),
    [state?.passport1]
  );
  const passport2Url = useMemo(
    () => (state?.passport2 ? URL.createObjectURL(state.passport2) : ''),
    [state?.passport2]
  );
  const locationScreenShotUrl = useMemo(
    () =>
      state?.locationScreenShot
        ? URL.createObjectURL(state.locationScreenShot)
        : '',
    [state?.locationScreenShot]
  );
    ...
    >
      ...
      <Box className="application-fields">
        ...
        <div className="application-field">
          ...
          <span className="application-field-value">
            {state?.house?.name || state?.exactAddress || '-'}
          </span>
        </div>
        {addressType === 'flat' && (
          <>
        ...
        <div className="application-field">
          <span className="application-field-title">
